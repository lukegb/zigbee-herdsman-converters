...
async function getNewImage(current, device, getImageMeta, downloadImage, suppressElementImageParseFailure) {
    ...
    if (checksum) {
        const hash = crypto_1.default.createHash(checksum.opensslHashName);
        hash.update(download.data);
        (0, assert_1.default)(hash.digest('hex') === checksum.hash, `File checksum validation failed`);
